...
const worker = new Worker('code-queue', async (job) => {
  ...
  try {
    ...
    await container.start();                                              // 2. Start execution and stream logs              

   
  const stream = await container.logs({                                   // Inside the worker loop...
    follow: true,
    stdout: true,
    stderr: true
  });
  ...
  stream.on('data', (chunk) => {
    ...
    redisPub.publish('job-logs', JSON.stringify({                        // Shout the log to Redis
      jobId: job.id,
      log: log
    }));
  });
    ...
    const waitPromise = container.wait();                                // 3. Wait for execution and set a timeout (Safety!)
    ...
    const logs = await container.logs({ stdout: true, stderr: true });   // 4. Capture logs
    const output = logs.toString().replace(/[\x00-\x1F\x7F-\x9F]/g, ""); // Clean Docker stream headers

   
    await container.remove();                                            // 5. Cleanup
